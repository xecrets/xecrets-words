...
internal class TrigramStrings(Trigrams trigrams)
{
    public string[] Starting { get; } = Keys(trigrams.Starting);

    public string[] Middle { get; } = Keys(trigrams.Middle);

    public string[] Ending { get; } = Keys(trigrams.Ending);

    private static string[] Keys(Dictionary<string, int> dictionary)
    {
